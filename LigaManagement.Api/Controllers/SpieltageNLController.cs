...
namespace LigaManagement.Api.Controllers
{
    ...
    [ApiController]
    public class SpieltageNLController : ControllerBase
    {
        private readonly ISpieltageNLRepository SpieltagNLRepository;

        public SpieltageNLController(ISpieltageNLRepository SpieltagNLRepository)
        {
            this.SpieltagNLRepository = SpieltagNLRepository;
        }
        ...
        public async Task<ActionResult> GetSpieltage()
        {
            try
            {
                return Ok(await SpieltagNLRepository.GetSpieltage());
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<Spieltag>> GetSpieltag(int id)
        {
            try
            {
                var result = await SpieltagNLRepository.GetSpieltag(id);
        ...
        public async Task<ActionResult<Spieltag>> CreateSpieltag(Spieltag spieltag)
        {
            try
            {
                ...
                var createdSpieltag = await SpieltagNLRepository.AddSpieltag(spieltag);
        ...
        public async Task<ActionResult<Spieltag>> UpdateSpieltag(Spieltag Spieltag)
        {
            try
            {
                var VereinToUpdate = await SpieltagNLRepository.GetSpieltag((int)Spieltag.SpieltagId);
                ...
                return await SpieltagNLRepository.UpdateSpieltag(Spieltag);
            }
            catch (Exception ex)
            {
                Debug.Print(ex.StackTrace);
        ...
        public async Task<ActionResult<Spieltag>> DeleteSpieltag(int id)
        {
            try
            {
                var spieltagToDelete = await SpieltagNLRepository.GetSpieltag(id);
                ...
                return await SpieltagNLRepository.DeleteSpieltag(id);
            }
            catch (Exception ex)
            {
                return StatusCode(StatusCodes.Status500InternalServerError,

...
namespace LigaManagement.Api.Controllers
{
    ...
    {
        private readonly ISpieltageBERepository SpieltageBERepository;

        public SpieltageBEController(ISpieltageBERepository SpieltagBERepository)
        {
            this.SpieltageBERepository = SpieltagBERepository;
        }
        ...
        public async Task<ActionResult> GetSpieltage()
        {
            try
            {
                return Ok(await SpieltageBERepository.GetSpieltage());
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<Spieltag>> GetSpieltag(int id)
        {
            try
            {
                var result = await SpieltageBERepository.GetSpieltag(id);
        ...
        public async Task<ActionResult<Spieltag>> CreateSpieltag(Spieltag spieltag)
        {
            try
            {
                ...
                var createdSpieltag = await SpieltageBERepository.AddSpieltag(spieltag);
        ...
        public async Task<ActionResult<Spieltag>> UpdateSpieltag(Spieltag Spieltag)
        {
            try
            {
                var VereinToUpdate = await SpieltageBERepository.GetSpieltag((int)Spieltag.SpieltagId);
                ...
                return await SpieltageBERepository.UpdateSpieltag(Spieltag);
            }
            catch (Exception ex)
            {
                Debug.Print(ex.StackTrace);
        ...
        public async Task<ActionResult<Spieltag>> DeleteSpieltag(int id)
        {
            try
            {
                var spieltagToDelete = await SpieltageBERepository.GetSpieltag(id);
                ...
                return await SpieltageBERepository.DeleteSpieltag(id);
            }
            catch (Exception ex)
            {
                return StatusCode(StatusCodes.Status500InternalServerError,

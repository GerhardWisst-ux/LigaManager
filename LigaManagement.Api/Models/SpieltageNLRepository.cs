...
{
    public class SpieltageNLRepository : ISpieltageNLRepository
    {
        public async Task<Spieltag> AddSpieltag(Spieltag spieltag)
        {
            try
            {
                ...
                cmd.Connection = conn;
                cmd.CommandText = "INSERT INTO SpieltageNL ([SpieltagNr],[Saison],[SaisonID],[LigaID],[Verein1_Nr],[Verein1],[Verein2_Nr],[Verein2],[Tore1_Nr],[Tore2_Nr],[Datum],[Ort],[Schiedrichter],[Abgeschlossen],[Zuschauer])" +
        ...
        public async Task<Spieltag> GetSpieltag(int spieltagId)
        {
            try
            {
                ...
                SqlCommand command = new SqlCommand("SELECT * FROM [SpieltageNL] WHERE SpieltagId =" + spieltagId, conn);
        ...
        public async Task<IEnumerable<Spieltag>> GetSpieltage()
        {
            ...
            try
            {
                ...
                SqlCommand command = new SqlCommand("SELECT * FROM [SpieltageNL] ", conn);
                Spieltag spieltag = null;
                List<Spieltag> Spieltaglist = new List<Spieltag>();
                using (SqlDataReader reader = command.ExecuteReader())
                {
        ...
        public int AktSpieltag(int SaisonID)
        {
            ...
            SqlCommand command = new SqlCommand("SELECT Max([SpieltagNr] +0) AS MAXSPIELTAG FROM [SpieltageNL] WHERE Datum<GETDATE() and SaisonID = '" + SaisonID + "'", conn);
        ...
        public async Task<Spieltag> UpdateSpieltag(Spieltag spieltag)
        {
            ...
            try
            {
                ...
                cmd.CommandText = "UPDATE [dbo].[SpieltageNL] SET " +

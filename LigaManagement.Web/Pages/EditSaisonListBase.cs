...
{
    public class EditSaisonListBase : ComponentBase
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            try
            {
                ...
                var VereineSaison = (await VereineService.GetVereineSaison()).Where(x => x.SaisonID == Convert.ToInt32(SaisonId)).OrderBy(x => x.VereinNr).ToList();
                ...
                foreach (var item in VereineSaison)
                {
                    var verein = await VereineService.GetVerein(item.VereinNr);
                    
                    if (verein != null)
                       ToggleSelection(new DisplayVerein(verein.VereinNr.ToString(), verein.Vereinsname1, true));                    
                }
